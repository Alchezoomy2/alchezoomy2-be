    ...
    await Promise.all(
      meetingData.map(meeting => {
        return client.query(`
                ...
                `,
          [meeting.uuid, meeting.host_id, meeting.topic, meeting.start_time, meeting.share_url, meeting.duration, meeting.video_play_url, meeting.audio_play_url, meeting.transcript_url, meeting.chat_file, meeting.meeting_views, meeting.meeting_fav]);
      })
    );
    ...
    await Promise.all(
      favData.map(favorite => {
        ...
        return client.query(`
                ...
                `,
          [favorite.uuid, favorite.host_id, favorite.topic, favorite.start_time, favorite.timestamp, favorite.speaker, favorite.text, favorite.owner_id]);
      })
    );
