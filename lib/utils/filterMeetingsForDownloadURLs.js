  ...
  obj.meetings.map(async (child) => {
    ...
    for (let key of child.recording_files) {
      if (key.recording_type === "audio_transcript") {
        ...
        console.log("inside the transcript url");
        try {
    ...
    const textFile = await fetch.get()
