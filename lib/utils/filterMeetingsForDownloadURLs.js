...
module.exports = async (obj) => {
  obj.meetings.forEach(async (child) => {
    ...
    for (let list of child.recording_files) {
      if (list.recording_type === "audio_transcript") {
        transcript_download_url = `${list.download_url}?access_token=${zoom_token}`;
      ...
      if (list.file_type === "MP4") {
        video_download_url = list.play_url;
      }
      if (list.recording_type === "audio_only") {
        audio_download_url = `${list.download_url}?access_token=${zoom_token}`;
      }
      chat_download_url = `${list.download_url}?access_token=${zoom_token}`;
    }
    await client.query(
      `
