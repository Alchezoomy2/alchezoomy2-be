...
app.get("/meetings", async (req, res) => {
  try {
    ...
    const promises = await Promise.all(
      formattedUrls.map((url) => nfetch(url))
    ).then((responses) =>
      Promise.all(responses.map((res) => res.text())).then(
        (text) => console.log(text)
        // this solution console logs allllllll of the transcript text to the console
      )
    );

    // const parsed = webvtt.parse(promises);
