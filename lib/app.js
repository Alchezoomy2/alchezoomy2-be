...
const insertNewChat = require('./utils/insertNewChat.js');
...
app.post('/meetings/', async (req, res) => {
  ...
  const teacher_info = req.body.teacher_info;
  ...
  try {
    const returnedMeetingObject = await fetch
      ...
      .set('Authorization', 'Bearer ' + teacher_info.access_token);

    const parsedMeetingArray = await parseMeetingsForListing(returnedMeetingObject.body, teacher_info);
...
app.post('/new_teacher/', async (req, res) => {
  ...
  // we should add error handling to check it Teacher currently exsists

  try {
    const returnedTeacherInfo = await insertNewTeacher(newTeacherInfo);
    res.status(200).json(returnedTeacherInfo)
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/publish/', async (req, res) => {
  ...
  const meetingObj = req.body;
  let teacherId;

  console.log(meetingObj.host_id);

  try {
    teacherId = await client.query(`
      SELECT 1
      FROM teachers
      WHERE teachers.host_id = $1`,
      [meetingObj.host_id]);

    console.log('teacherId.rows[0]');
    console.log(teacherId.rows[0]);

    const meetingId = await insertNewMeeting(meetingObj, teacherId.rows[0].id);

    if (meetingObj.chat_download_url != '') {
      await insertNewChat(meetingObj, teacherId, meetingId);
    }

    if (meetingObj.transcript_download_url != '') {
      await insertNewTranscript(meetingObj, teacherId, meetingId);
    }
