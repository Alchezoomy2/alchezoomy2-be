...
app.get('/seed_meetings', async(req, res) => {
  try {
    const zoom_url =
      'https://api.zoom.us/v2/users/Wq3hOAd6QsSDGuYInNBdyw/recordings/?from=2020-11-08&to=2020-11-09';
...
app.post('/api/bookmarks', async(req, res) => {
  ...
  try {
    ...
    app.delete('/api/bookmarks/:id', async(req, res) => {
      try {
        const bookmarks_uuid = req.params.id;
        const data = await client.query(
          `
          DELETE from bookmarks
          WHERE bookmarks.uuid = $1
          AND bookmarks.owner_id = $2
          RETURNING *`,
          [bookmarks_uuid, req.userId]
        );
        res.json(data.rows);
      } catch(e) {
        res.status(500).json({ error: e.message });
