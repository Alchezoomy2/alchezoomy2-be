...
const addBookmark = require('./utils/addBookmark.js');
const retrieveBookmarks = require('./utils/retrieveBookmarks.js');
...
app.post('/student/bookmark', async (req, res) => {
  ...
  try {
    await addBookmark(chatId, studentId);
    const bookmarks = await retrieveBookmarks(studentId);
