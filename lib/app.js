...
app.post('/oauth', async (req, res) => {
  ...
  try {
    ...
    // const header = { 'Authorization': 'Basic QXhyYkg4M19RMGFFTzI3M2RGSWFmdzo4UmxBQXZZMm5DSVZ6ZU5ZOE44T2laVE1FM3p3aDg5ZQ==' };


    let returnedObject = await fetch
      .post(acccess_token_url)
      .set('Authorization', 'Basic QXhyYkg4M19RMGFFTzI3M2RGSWFmdzo4UmxBQXZZMm5DSVZ6ZU5ZOE44T2laVE1FM3p3aDg5ZQ==');

    const oauthToken = returnedObject.body.access_token;
    // console.log('-----------------')
    // console.log(returnedObject.body)
    // console.log('-----------------')
    let returnedUserInfo = await fetch
      .get(zoom_url + 'users/me')
      .set('Authorization', 'Bearer ' + oauthToken);
    // console.log('------------------------------------');
    // console.log(returnedUserInfo.body);
    // console.log('------------------------------------');

    const parsedTeacherInfo = await parseReturnedTeacherInfo(returnedUserInfo.body, oauthToken);

    // console.log('------------------------------------');
    // console.log(parsedTeacherInfo.promise);
    // console.log('------------------------------------');
...
app.post('/meetings/:mType', async (req, res) => {

  if (req.params.mType === 'unpublished') {
    const access_token = req.body.access_token;

    try {
      const returnedMeetingObject = await fetch
        .get(zoom_url + 'users/me/recordings')
        .set('Authorization', 'Bearer ' + access_token);

      console.log('------------------------------------');
      console.log(returnedMeetingObject.body);
      console.log('------------------------------------');

      const parsedMeetingObject = parseMeetingsForListing(returnedMeetingObject.body, access_token);

      console.log('------------------------------------');
      console.log(parsedMeetingObject);
      console.log('------------------------------------');
      res.status(200).json(parsedMeetingObject);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
