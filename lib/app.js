...
app.post('/oauth/', async (req, res) => {
  ...
  try {
    const returnedObject = await fetch.post(access_token_url).set('Authorization', 'Basic QXhyYkg4M19RMGFFTzI3M2RGSWFmdzo4UmxBQXZZMm5DSVZ6ZU5ZOE44T2laVE1FM3p3aDg5ZQ==');

    access_token = returnedObject.body.access_token;

    const userInfo_URL = "https://api.zoom.us/v2/users/me";

    const returnedUserInfo = await fetch.get(userInfo_URL)
