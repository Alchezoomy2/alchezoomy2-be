...
app.post('/meetings/:mType', async (req, res) => {
  ...
  if (req.params.mType === 'unpublished') {
    ...
    try {
      ...
      const parsedMeetingArray = await parseMeetingsForListing(returnedMeetingObject.body, user_info);
...
app.post('/publish/', async (req, res) => {
  ...
  const meetingObj = req.body.meetingObj;
  let teacher_id;

  try {
    if (meetingObj.new_user === true) {
      console.log('new user! !!!!!!!!!!!!!!!!!!!!!!!!!!');
      teacher_id = await insertNewTeacher(meetingObj);
    }

    const insertedMeeting = await insertNewMeeting(meetingObj, teacher_id);

    res.status(200).json(insertedMeeting);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
