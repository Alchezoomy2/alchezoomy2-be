...
app.post('/oauth', async (req, res) => {
  ...
  try {
    ...
    console.log('------------------------------------');
    console.log(`returnedUserInfo.body:  ${returnedUserInfo.body}`);
    console.log('------------------------------------');

    const parsedTeacherInfo = parseReturnedTeacherInfo(returnedUserInfo.body, oauthToken);

    console.log('------------------------------------');
    console.log(`parsedTeacherInfo:  ${parsedTeacherInfo}`);
    console.log('------------------------------------');
  ...
  if (req.params.mType === 'unpublished') {
    ...
    try {
      const returnedMeetingObject = await fetch
        .get(zoom_token + 'users/{userId}/recordings')
        .set('Authorixation', 'Bearer ' + access_token);

      console.log(returnedMeetingObject.body);

      res.status(200).json(returnedMeetingObject.body);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
