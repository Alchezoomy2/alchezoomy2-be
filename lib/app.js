...
const filterMeetingsForDownloadUrls = require('./utils/filterMeetingsForDownloadURLs.js');// const { filter } = require("minimatch");
...
app.get('/new_meeting', async (req, res) => {
...
app.get('/seed_meetings', async (req, res) => {
  try {
    const zoom_url =
      'https://api.zoom.us/v2/users/Wq3hOAd6QsSDGuYInNBdyw/recordings/?from=2020-11-09&to=2020-11-10';
...
// _recording_start_ returns an array of start times, can loop through to seed the database
// const recording_start = filterMeetingData(
//   returnedObject.body,
//   "recording_start"
// );

// // _recording_end_ returns an array of end times, can loop through to seed the database
// const recording_end = filterMeetingData(
//   returnedObject.body,
//   "recording_end"
// );

// const urls = filterMeetingData(returnedObject.body, "download_url");
...
// // _parsed_ returns an array of parsed and formatted node-webvtt objects, can loop through to seed the database
...
app.post('/api/favorites', async (req, res) => {
  ...
  try {
    const data = await client.query(
      ...
      RETURNING *`,
      [
        parseInt(new_fav.uuid),
        new_fav.host_id,
        new_fav.topic,
        parseInt(new_fav.start_time),
        parseInt(new_fav.timestamp),
        new_fav.speaker,
        new_fav.text,
        req.userId,
      ]
