...
module.exports = class Teacher {
    ...
    static async updateColor(teacherId, newColor) {
        ...
        RETURNING *
        `, [newColor, teacherId])
        console.log("🚀 ~ file: Teacher.js ~ line 136 ~ Teacher ~ updateColor ~ rows ", rows)

        const updatedTeacher = new Teacher(rows[0])

        const returnedMeetings = await client.query(`
        UPDATE meetings 
        SET color = $1
        WHERE teacher_id = $2
        RETURNING *
        ...
        console.log(returnedMeetings.rows)

        const updatedMeetings = returnedMeetings.rows.map(row => new Meeting(row))
