...
module.exports = class Meeting {
    ...
    static async publish(meetingId, accessToken) {
        ...
        if (meetingObj.videoUrl) {
            const videoPath = `videos/${meetingObj.teacherId}/${meetingObj.id}.mp4`;
            await sendToS3(videoPath, meetingObj.videoUrl, accessToken);
        }

        if (meetingObj.chatUrl) {
            console.log('chat_url')
            await insertNewChat(meetingObj, meetingId, accessToken);
        }

        if (meetingObj.transcriptUrl) {
