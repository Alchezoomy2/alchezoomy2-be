    ...
    static async loadNewMeetings(newMeetingArray, teacherInfo) {
        ...
        for (let newMeeting of newMeetingArray) {
            ...
            for (let oldMeeting of oldMeetingArray) {
                if (oldMeeting.startTime === newMeeting.start_time) {
                    console.log("🚀 ~ file: Meeting.js ~ line 80 ~ Meeting ~ loadNewMeetings ~ oldMeeting", oldMeeting)
                    if (oldMeeting.topic !== newMeeting.topic ||
                        oldMeeting.duration !== newMeeting.duration) {
                        const updatedMeeting = await parseMeetingObj(newMeeting, teacherInfo);
                        console.log("🚀 ~ file: Meeting.js ~ line 83 ~ Meeting ~ loadNewMeetings ~ updatedMeeting", updatedMeeting)
                        await this.update(oldMeeting.id, updatedMeeting)
                        this.update(oldMeeting.id, newMeeting)
