...
module.exports = class Meeting {
        ...
        for (let newMeeting of newMeetingArray) {
            ...
            if (existingMeeting) {
                ...
                if (existingMeeting.topic !== newMeeting.topic || existingMeeting.duration !== newMeeting.duration) {
                    ...
                    await this.updateLocally(existingMeeting.id, updatedMeeting)
                }
            }
    ...
    static async publish(meetingId, accessToken) {
        ...
        if (meetingObj.video_url) {
            console.log("🚀 ~ file: Meeting.js ~ line 171 ~ Meeting ~ publish ~ meetingObj.video_url", meetingObj.video_url)
            ...
            console.log("🚀 ~ file: Meeting.js ~ line 193 ~ Meeting ~ publish ~ meetingObj.audio_url", meetingObj.audio_url)
        ...
        if (meetingObj.chat_url) {
            console.log("🚀 ~ file: Meeting.js ~ line 190 ~ Meeting ~ publish ~ meetingObj.chat_url", meetingObj.chat_url)
    ...
    static async unpublish(meetingId) {
        ...
        RETURNING * `,
            [meetingId]);
        const videoPath = meetingObj.media_url.split("com/")[1]
