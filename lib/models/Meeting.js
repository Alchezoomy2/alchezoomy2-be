...
module.exports = class Meeting {
    ...
    static async checkOnZoom(teacherId, access_token) {
        console.log("🚀 ~ file: Meeting.js ~ line 139 ~ Meeting ~ checkOnZoom ~ teacherId, access_token", teacherId, access_token)
        try {
            const oldMeetingArray = await this.findAll(teacherId);
            console.log("🚀 ~ file: Meeting.js ~ line 141 ~ Meeting ~ checkOnZoom ~ oldMeetingArray", oldMeetingArray);
            ...
            for (let meeting of oldMeetingArray) {
                if (meeting.onZoom) {
                    const meetingId = await this.fetchZoomMeetingId(meeting.id);
            ...
            Promise.all(promises)
                .then(resultArray => {
                    for (let result of resultArray) {
                        console.log("🚀 ~ file: Meeting.js ~ line 160 ~ Meeting ~ checkOnZoom ~ result", result.status)
