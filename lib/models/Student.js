...
module.exports = class Student {
    ...
    static async resetPassword(studentId, password) {
        const newPasswordHash = await bcrypt.hash(password, Number(saltRounds));
        ...
        return { code: "error", message: "Student not found" }
    }
    ...
    static async findByEmail(studentEmail) {
        console.log("🚀 ~ file: Student.js ~ line 138 ~ Student ~ findByEmail ~ studentEmail", studentEmail)
        const { rows } = await client.query(`
            SELECT *
            FROM students
            WHERE student_email = $1
