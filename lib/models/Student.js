...
module.exports = class Student {
    ...
    static async addPermission({ studentEmail, teacherEmail }) {
        const returnedTeacherId = await client.query(`
        SELECT id
        FROM teachers
        WHERE email = $1
        `, [teacherEmail]);

        console.log(returnedTeacherId.rows[0])

        if (returnedTeacherId.rows[0].length > 0) {
            const returnedMeetingArray = await client.query(`
            UPDATE students
            SET permissions = array_append(permissions, $1)
            WHERE email=$2
            `, [returnedTeacherId.rows[0].id, studentEmail])

            return returnedMeetingArray.rows.map(meeting => new Meeting(meeting))
        }
