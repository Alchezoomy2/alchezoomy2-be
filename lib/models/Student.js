...
module.exports = class Student {
    ...
    static async login(studentEmail, password) {
        console.log("🚀 ~ file: Student.js ~ line 77 ~ Student ~ login ~ studentEmail, password", studentEmail, password)
        ...
        `, [studentEmail])
        console.log("🚀 ~ file: Student.js ~ line 85 ~ Student ~ login ~ rows", rows)


        const passwordsMatch = await bcrypt.compare(password, rows[0].password_hash);
        console.log("🚀 ~ file: Student.js ~ line 88 ~ Student ~ login ~ passwordsMatch", passwordsMatch)

        if (passwordsMatch) {
            return new Student(row[0]);
