...
module.exports = Router()
    ...
    .post('/', ensureTeacherAuth, async (req, res, next) => {
        ...
        console.log("🚀 ~ file: teacher-meetings-routes.js ~ line 15 ~ .post ~ cookie", cookie)

        try {
            const { access_token, refresh_token, token_created } = await refreshToken(cookie);
            console.log("🚀 ~ file: teacher-meetings-routes.js ~ line 18 ~ .post ~ access_token, refresh_token, token_created ", access_token, refresh_token, token_created)
            ...
            if (returnedMeetingArray.meetings.length > 0) {
                ...
                console.log("🚀 ~ file: teacher-meetings-routes.js ~ line 30 ~ .post ~ completeMeetingList", completeMeetingList)
