...
module.exports = Router()
    ...
    .post('/oauth', async (req, res, next) => {
        try {
            if (req.body.jwt) {
                const valid = InviteHash.verifyToken(req.body.jwt)
                console.log("🚀 ~ file: teacher-routes.js ~ line 34 ~ .post ~ valid", valid.teacherEmail)
                if (valid) {
                    const teacherInfo = teacherAuth.authorizeNew(req.body)
                    ...
                    res.json({ message: "Invitation invalid" })
                }
            } else {
