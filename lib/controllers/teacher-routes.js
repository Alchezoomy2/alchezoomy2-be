...
module.exports = Router()
    ...
    .get('/invite/:email', ensureTeacherAuth, async (req, res) => {
        try {
            const inviteEmail = req.params.email;

            const inviteHash = InviteHash.createToken(inviteEmail)
            const response = inviteStudent(inviteEmail.split(':')[0], inviteHash)
