    ...
    .post('/oauth', async (req, res, next) => {
        ...
        try {
            if (code) {
                ...
                console.log("🚀 ~ file: teacher-routes.js ~ line 21 ~ .post ~ teacherObj, access_token, refresh_token", teacherObj, access_token, refresh_token)

                const parsedTeacherInfo = await parseReturnedTeacherInfo(teacherObj, access_token, refresh_token)
                console.log("🚀 ~ file: teacher-routes.js ~ line 25 ~ .post ~ parsedTeacherInfo ", parsedTeacherInfo)
