...
module.exports = Router()
    ...
    .get('/reset/:email', async (req, res) => {
        ...
        try {
            ...
            console.log("🚀 ~ file: student-routes.js ~ line 47 ~ .get ~ found", found)

            if (found) {
                const resetHash = ResetHash.createToken(studentEmail)
                const response = await resetStudentPassword(studentEmail, resetHash)
                console.log("🚀 ~ file: student-routes.js ~ line 53 ~ .post ~ response", response)

                res.json(response)
            } else {
                res.json({ code: "warning", message: "Email address not found" })
            }
        } catch (e) {
